...
INVERTER::INVERTER(const GraphicsInfo &r_GfxInfo, int r_FanOut)
    : Gate(1, r_FanOut) {
...
int INVERTER::GetOutPinStatus() { return m_OutputPin.getStatus(); }
...
void INVERTER::Save(ofstream &fout) {
  ...
  if (label == "")
    label = "$";
  fout << "INVERTER " << GetID() << " " << label << " " << m_GfxInfo.x1 << " "
       << m_GfxInfo.y1 << endl;
}
