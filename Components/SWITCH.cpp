...
void SWITCH::Draw(Output *pOut) {
  ...
  bool isOn = (m_Status == HIGH);
  pOut->DrawSWITCH(m_GfxInfo, IsSelected(), GetLabel(), isOn);
}

// returns status of outputpin
int SWITCH::GetOutPinStatus() { return m_OutputPin.getStatus(); }
...
void SWITCH::SetStatus(STATUS s) { m_Status = s; }

STATUS SWITCH::GetStatus() { return m_Status; }
...
OutputPin *SWITCH::GetOutputPin() { return &m_OutputPin; }

void SWITCH::Save(ofstream &fout) {
  string label = GetLabel();
  if (label == "")
    label = "$";
  fout << "SWITCH " << GetID() << " " << label << " " << m_GfxInfo.x1 << " "
       << m_GfxInfo.y1 << " " << m_Status << endl;
}
